...
def seed():
    ...
    # ── Users ──────────────────────────────────────────────
    # SECURITY: passwords are never hardcoded here. Each one comes from an
    # environment variable (set it in your .env / Railway config before running
    # this script). If a var is missing, a strong random password is generated
    # and printed ONCE at the end — write it down, it cannot be recovered later.
    generated_passwords = {}
    ...
    def _resolve_password(env_var: str) -> str:
        ...
        generated_passwords[env_var] = pw
        return pw

    user_defs = [
        ("cashier",    "Badr bin Sheehon",  "cashier",    "SEED_CASHIER_PASSWORD"),
        ("supervisor", "Mohammed Adel",     "supervisor", "SEED_SUPERVISOR_PASSWORD"),
        ("admin",      "Tareq",             "admin",      "SEED_ADMIN_PASSWORD"),
    ]
    for username, full_name, role_name, env_var in user_defs:
        u = db.query(User).filter(User.username == username).first()
        if not u:
            password = _resolve_password(env_var)
            u = User(
                username=username,
                full_name=full_name,
    ...
    if generated_passwords:
        print("⚠  The following passwords were AUTO-GENERATED because no env var was set.")
        print("   Save them now — they are not stored anywhere and cannot be shown again:")
        for env_var, pw in generated_passwords.items():
            print(f"   {env_var} → {pw}")
        print()
        print("   To set your own passwords next time, export these before running seed.py:")
        print("   SEED_ADMIN_PASSWORD, SEED_SUPERVISOR_PASSWORD, SEED_CASHIER_PASSWORD")
    else:
        print("Users created using the passwords from your environment variables.")
